...
DEVICEDIR=`find device -type d -name $DEVICE`

OSRZIP=`grep ro.modversion $SCRIPTDIR/../products/team_$DEVICE.mk | cut -f 2 -d "=" | cut -f 1 -d " "`
OSRVER=`grep ro.stats.romversion $SCRIPTDIR/../products/team_$DEVICE.mk | cut -f 2 -d "=" | cut -f 1 -d " "`
...
rm ../update.zip
zip -qr ../update.zip .
UPDATEFILE=$OUT/SuperOSR-$DEVICE-$OSRVER.zip
cd $TOPDIR
